...
# Beep Selector - Test Beep
# Plays the currently configured start beep for testing

CONFIG_FILE="/usr/local/etc/beep-selector/beepselector.conf"
BEEPS_DIR="/usr/local/etc/beep-selector/beeps"
...
case "$start_beep" in
    ...
    custom)
        ...
        ;;
    *)
        beep_file="${BEEPS_DIR}/${start_beep}.spkr"
        ;;
esac
